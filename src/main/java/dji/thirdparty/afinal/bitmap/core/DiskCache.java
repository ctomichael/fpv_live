...
public class DiskCache implements Closeable {
    ...
    private boolean lookupInternal(long key, int hashStart) {
        int slot;
        int slot2 = (int) (key % ((long) this.mMaxEntries));
        if (slot2 < 0) {
            slot2 += this.mMaxEntries;
        }
        int slotBegin = slot;
        while (true) {
    ...
    /* access modifiers changed from: package-private */
    public int checkSum(byte[] data, int offset, int nbytes) {
        this.mAdler32.reset();
    ...
    static int readInt(byte[] buf, int offset) {
        return (buf[offset] & 255) | ((buf[offset + 1] & 255) << 8) | ((buf[offset + 2] & 255) << Tnaf.POW_2_WIDTH) | ((buf[offset + 3] & 255) << 24);
    }
