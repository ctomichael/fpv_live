package dji.midware.data.model.P3;

import android.support.annotation.Keep;
import dji.fieldAnnotation.EXClassNullAway;
import dji.midware.data.config.P3.CmdIdCamera;
import dji.midware.data.config.P3.CmdSet;
import dji.midware.data.config.P3.DataConfig;
import dji.midware.data.config.P3.DeviceType;
import dji.midware.data.manager.P3.DataBase;
import dji.midware.data.model.P3.DataCameraSetVOutParams;
import dji.midware.data.packages.P3.SendPack;
import dji.midware.interfaces.DJIDataCallBack;
import dji.midware.interfaces.DJIDataSyncListener;

@Keep
@EXClassNullAway
public class DataCameraGetVOutParams extends DataBase implements DJIDataSyncListener {
    private static DataCameraGetVOutParams instance = null;

    public static synchronized DataCameraGetVOutParams getInstance() {
        DataCameraGetVOutParams dataCameraGetVOutParams;
        synchronized (DataCameraGetVOutParams.class) {
            if (instance == null) {
                instance = new DataCameraGetVOutParams();
            }
            dataCameraGetVOutParams = instance;
        }
        return dataCameraGetVOutParams;
    }

    public DataCameraSetVOutParams.LCDFormat getLCDFormat(int index) {
        return DataCameraSetVOutParams.LCDFormat.find(((Integer) get(0, 1, Integer.class, index)).intValue());
    }

    public boolean isHDOpen() {
        return isHDOpen(-1);
    }

    public boolean isHDOpen(int index) {
        return ((Integer) get(0, 1, Integer.class, index)).intValue() == DataCameraSetVOutParams.LCDFormat.HD_FORMAT.ordinal();
    }

    public int getStream() {
        return getStream(-1);
    }

    public int getStream(int index) {
        return ((Integer) get(2, 1, Integer.class, index)).intValue();
    }

    public void start(DJIDataCallBack callBack) {
        SendPack pack = new SendPack();
        pack.senderType = DeviceType.APP.value();
        pack.receiverType = DeviceType.CAMERA.value();
        pack.cmdType = DataConfig.CMDTYPE.REQUEST.value();
        pack.isNeedAck = DataConfig.NEEDACK.YES.value();
        pack.encryptType = DataConfig.EncryptType.NO.value();
        pack.cmdSet = CmdSet.CAMERA.value();
        pack.cmdId = CmdIdCamera.CmdIdType.GetVOutParams.value();
        start(pack, callBack);
    }

    /* access modifiers changed from: protected */
    public void doPack() {
    }
}
