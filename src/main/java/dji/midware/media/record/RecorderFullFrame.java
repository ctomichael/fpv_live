...
@EXClassNullAway
public class RecorderFullFrame extends RecorderBase implements RecorderInterface, FullFrameHardwareTranscoder.FullFrameTranscoderListener {
    ...
    public static synchronized RecorderFullFrame getInstance() {
        RecorderFullFrame recorderFullFrame;
        synchronized (RecorderFullFrame.class) {
            if (instance == null) {
                instance = new RecorderFullFrame();
                DJIEventBusUtil.register(instance);
            }
            recorderFullFrame = instance;
        }
        return recorderFullFrame;
    }
    ...
    public synchronized void onFrameInput(ByteBuffer buffer, MediaCodec.BufferInfo info, int width, int height, boolean isKeyFrame) {
        if (checkIsCurrentRecorder()) {
            StreamDataObserver.getInstance(StreamDataObserver.ObservingPoint.RecorderFullFrameInput).onDataRecv(StreamDataObserver.ObservingContext.FrameRate, 1).onDataRecv(StreamDataObserver.ObservingContext.ByteRate, (long) info.size).onDataRecv(StreamDataObserver.ObservingContext.KeyframeNum, (info.flags & 1) > 0 ? 1 : 0).onDataRecv(StreamDataObserver.ObservingContext.Width, (long) width).onDataRecv(StreamDataObserver.ObservingContext.Height, (long) height);
            logDataInput("fullframe frame input count=" + this.numFrameWritten, DJIVideoDecoder.connectLosedelay, RecorderBase.LogDataMode.Video);
            ...
            if (this.last_written_pts >= 0) {
                ...
                info.presentationTimeUs = this.last_written_pts + Math.min(41667L, Math.max(DJIVideoUtil.getDurationPerFrameUs(), (this.initial_original_pts + (1000 * (time - this.initialOriginalTime))) - this.last_written_pts));
            }
