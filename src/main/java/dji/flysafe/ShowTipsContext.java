...
public final class ShowTipsContext extends Message<ShowTipsContext, Builder> {
    ...
    @WireField(adapter = "dji.flysafe.TipsEventType#ADAPTER", tag = 1)
    public final TipsEventType type;

    public ShowTipsContext(TipsEventType type2, List<LimitArea> areas2, Double limitHeight2) {
        this(type2, areas2, limitHeight2, ByteString.EMPTY);
    }

    public ShowTipsContext(TipsEventType type2, List<LimitArea> areas2, Double limitHeight2, ByteString unknownFields) {
    ...
    public boolean equals(Object other) {
        ...
        if (!(other instanceof ShowTipsContext)) {
            return false;
        }
        ShowTipsContext o = (ShowTipsContext) other;
    ...
    public String toString() {
        ...
        return builder.replace(0, 2, "ShowTipsContext{").append('}').toString();
    }

    public static final class Builder extends Message.Builder<ShowTipsContext, Builder> {
        public List<LimitArea> areas = Internal.newMutableList();
        public Double limitHeight;
        public TipsEventType type;

        public Builder type(TipsEventType type2) {
            this.type = type2;
            return this;
        }
        ...
        public ShowTipsContext build() {
            return new ShowTipsContext(this.type, this.areas, this.limitHeight, super.buildUnknownFields());
        }
    }

    private static final class ProtoAdapter_ShowTipsContext extends ProtoAdapter<ShowTipsContext> {
        ProtoAdapter_ShowTipsContext() {
            super(FieldEncoding.LENGTH_DELIMITED, ShowTipsContext.class);
        }

        public int encodedSize(ShowTipsContext value) {
            int i = 0;
            int encodedSizeWithTag = (value.type != null ? TipsEventType.ADAPTER.encodedSizeWithTag(1, value.type) : 0) + LimitArea.ADAPTER.asRepeated().encodedSizeWithTag(2, value.areas);
        ...
        public void encode(ProtoWriter writer, ShowTipsContext value) throws IOException {
            if (value.type != null) {
                TipsEventType.ADAPTER.encodeWithTag(writer, 1, value.type);
            }
        ...
        public ShowTipsContext decode(ProtoReader reader) throws IOException {
            Builder builder = new Builder();
            ...
            while (true) {
                ...
                if (tag != -1) {
                    switch (tag) {
                        case 1:
                            try {
                                builder.type(TipsEventType.ADAPTER.decode(reader));
                                break;
        ...
        public ShowTipsContext redact(ShowTipsContext value) {
