...
public class ECAlgorithms {
    ...
    private static ECPoint implSumOfMultiplies(boolean[] zArr, WNafPreCompInfo[] wNafPreCompInfoArr, byte[][] bArr) {
        ...
        while (i3 >= 0) {
            ...
            while (i5 < length) {
                byte[] bArr3 = bArr[i5];
                byte b = i3 < bArr3.length ? bArr3[i3] : 0;
                if (b != 0) {
                    int abs = Math.abs((int) b);
                    WNafPreCompInfo wNafPreCompInfo = wNafPreCompInfoArr[i5];
                    eCPoint = eCPoint3.add(((b < 0) == zArr[i5] ? wNafPreCompInfo.getPreComp() : wNafPreCompInfo.getPreCompNeg())[abs >>> 1]);
                } else {
                    eCPoint = eCPoint3;
                }
                i5++;
            ...
            if (eCPoint3 == infinity) {
                i = i4 + 1;
            } else {
                if (i4 > 0) {
                    i = 0;
                    eCPoint2 = eCPoint2.timesPow2(i4);
                } else {
                    i = i4;
                }
        ...
        return i4 > 0 ? eCPoint2.timesPow2(i4) : eCPoint2;
    }
    ...
    static ECPoint implSumOfMultipliesGLV(ECPoint[] eCPointArr, BigInteger[] bigIntegerArr, GLVEndomorphism gLVEndomorphism) {
        ...
        int i = 0;
        for (int i2 = 0; i2 < length; i2++) {
            BigInteger[] decomposeScalar = gLVEndomorphism.decomposeScalar(bigIntegerArr[i2].mod(order));
            int i3 = i + 1;
        ...
        ECPoint[] eCPointArr2 = new ECPoint[(length << 1)];
        int i4 = 0;
        for (ECPoint eCPoint : eCPointArr) {
            ECPoint map = pointMap.map(eCPoint);
            int i5 = i4 + 1;
            eCPointArr2[i4] = eCPoint;
            i4 = i5 + 1;
            eCPointArr2[i5] = map;
        }
