...
public abstract class Mod {
    ...
    public static void invert(int[] iArr, int[] iArr2, int[] iArr3) {
        ...
        if (Nat.isZero(length, iArr2)) {
            ...
            int i2 = length;
            int i3 = inversionStep;
            while (true) {
                if (copy[i2 - 1] == 0 && copy2[i2 - 1] == 0) {
                    i2--;
                } else if (Nat.gte(i2, copy, copy2)) {
                    Nat.subFrom(i2, copy2, copy);
                    int inversionStep2 = inversionStep(iArr, copy, i2, create, (Nat.subFrom(length, create2, create) - i) + i3);
                    ...
                    i = inversionStep(iArr, copy2, i2, create2, (Nat.subFrom(length, create, create2) - i3) + i);
