...
public class GMSSSigner implements MessageSigner {
    ...
    private void initSign() {
        ...
        if (gMSSPrivateKeyParameters.isUsed()) {
            ...
            for (int i = 0; i < this.numLayer; i++) {
                this.currentAuthPaths[i] = (byte[][]) Array.newInstance(Byte.TYPE, currentAuthPaths2[i].length, this.mdLength);
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        while (i >= 0) {
            ...
            i2 = i3 + signatureLength;
            byte[] Verify = winternitzOTSVerify.Verify(bArr, bArr3);
            if (Verify == null) {
                System.err.println("OTS Public Key is null in GMSSSignature.verify");
                return false;
            }
            byte[][] bArr4 = (byte[][]) Array.newInstance(Byte.TYPE, this.gmssPS.getHeightOfTrees()[i], this.mdLength);
            for (byte[] bArr5 : bArr4) {
                System.arraycopy(bArr2, i2, bArr5, 0, this.mdLength);
                i2 += this.mdLength;
            }
            byte[] bArr6 = new byte[this.mdLength];
            int length = bytesToIntLittleEndian + (1 << bArr4.length);
            for (int i4 = 0; i4 < bArr4.length; i4++) {
                byte[] bArr7 = new byte[(this.mdLength << 1)];
                if (length % 2 == 0) {
                    System.arraycopy(Verify, 0, bArr7, 0, this.mdLength);
                    System.arraycopy(bArr4[i4], 0, bArr7, this.mdLength, this.mdLength);
                } else {
                    System.arraycopy(bArr4[i4], 0, bArr7, 0, this.mdLength);
                    System.arraycopy(Verify, 0, bArr7, this.mdLength, Verify.length);
                    length--;
                }
                length /= 2;
