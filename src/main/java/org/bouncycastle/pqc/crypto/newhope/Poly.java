...
class Poly {
    ...
    static void getNoise(short[] sArr, byte[] bArr, byte b) {
        ...
        for (int i = 0; i < 1024; i++) {
            ...
            sArr[i] = (short) (((((i2 >>> 24) + (i2 >>> 0)) & 255) + 12289) - (((i2 >>> 8) + (i2 >>> 16)) & 255));
        }
    }
    ...
    static void pointWise(short[] sArr, short[] sArr2, short[] sArr3) {
        for (int i = 0; i < 1024; i++) {
            sArr3[i] = Reduce.montgomery((sArr[i] & UShort.MAX_VALUE) * (Reduce.montgomery((sArr2[i] & UShort.MAX_VALUE) * 3186) & UShort.MAX_VALUE));
        }
    }
