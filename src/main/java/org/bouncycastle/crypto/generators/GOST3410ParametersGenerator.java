...
public class GOST3410ParametersGenerator {
    ...
    private void procedure_B(int i, int i2, BigInteger[] bigIntegerArr) {
        ...
        while (true) {
            ...
            BigInteger add = TWO.pow(DJIDiagnosticsError.Camera.INTERNAL_STORAGE_INVALID).divide(bigInteger3.multiply(bigInteger4)).add(TWO.pow(DJIDiagnosticsError.Camera.INTERNAL_STORAGE_INVALID).multiply(bigInteger5).divide(bigInteger3.multiply(bigInteger4).multiply(TWO.pow(1024))));
            if (add.mod(TWO).compareTo(ONE) == 0) {
                add = add.add(ONE);
            }
            int i5 = 0;
