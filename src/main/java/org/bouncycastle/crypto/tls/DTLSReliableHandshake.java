...
class DTLSReliableHandshake {
    ...
    /* JADX DEBUG: Failed to find minimal casts for resolve overloaded methods, cast all args instead
     ...
     arg types: [short, org.bouncycastle.crypto.tls.DTLSReliableHandshake$RecordLayerBuffer]
     candidates:
    ...
    public Message receiveMessage() throws IOException {
        ...
        if (dTLSReassembler == null || (bodyIfComplete2 = dTLSReassembler.getBodyIfComplete()) == null) {
            int i3 = 1000;
            while (true) {
                        ...
                        if (receive < 0) {
                            ...
                            if (receive == readUint24 + 12 && (readUint16 = TlsUtils.readUint16(bArr, 4)) <= this.next_receive_seq + 10) {
                                ...
                                if (readUint243 + readUint24 > readUint242) {
                                    ...
                                    if (readUint16 == this.next_receive_seq && (bodyIfComplete = dTLSReassembler2.getBodyIfComplete()) != null) {
                                        this.previousInboundFlight = null;
                                        int i4 = this.next_receive_seq;
                                        this.next_receive_seq = i4 + 1;
                                        return updateHandshakeMessagesDigest(new Message(i4, dTLSReassembler2.getMsgType(), bodyIfComplete));
                                    }
                ...
                resendOutboundFlight();
                i3 = Math.min(i * 2, 60000);
            }
        } else {
            this.previousInboundFlight = null;
            int i5 = this.next_receive_seq;
            this.next_receive_seq = i5 + 1;
            return updateHandshakeMessagesDigest(new Message(i5, dTLSReassembler.getMsgType(), bodyIfComplete2));
        }
    }
