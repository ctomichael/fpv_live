...
public class SkipjackEngine implements BlockCipher {
    ...
    private int g(int i, int i2) {
        ...
        return (s4 << 8) + (s3 ^ ftable[this.key3[i] ^ s4]);
    }

    private int h(int i, int i2) {
        short s = (i2 >> 8) & 255;
        short s2 = (i2 & 255) ^ ftable[this.key3[i] ^ s];
        short s3 = s ^ ftable[this.key2[i] ^ s2];
        short s4 = s2 ^ ftable[this.key1[i] ^ s3];
        return s4 + ((s3 ^ ftable[this.key0[i] ^ s4]) << 8);
    }
