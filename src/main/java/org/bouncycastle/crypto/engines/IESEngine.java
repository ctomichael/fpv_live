...
public class IESEngine {
    ...
    private byte[] decryptBlock(byte[] bArr, int i, int i2) throws InvalidCipherTextException {
        ...
        if (this.cipher == null) {
            ...
            processBytes = this.cipher.processBytes(bArr, i + this.V.length, (i2 - this.V.length) - this.mac.getMacSize(), bArr2, 0);
            bArr3 = bArr7;
        }
        ...
        if (Arrays.constantTimeAreEqual(copyOfRange, bArr10)) {
            return this.cipher == null ? bArr2 : Arrays.copyOfRange(bArr2, 0, processBytes + this.cipher.doFinal(bArr2, processBytes));
        }
