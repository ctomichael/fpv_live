...
public class HC128Engine implements StreamCipher {
    ...
    private static int mod1024(int i) {
        return i & DJIDiagnosticsError.Camera.INTERNAL_STORAGE_INVALID;
    }
    ...
    private int step() {
        ...
        if (this.cnt < 512) {
            int[] iArr = this.p;
            iArr[mod512] = iArr[mod512] + g1(this.p[dim(mod512, 3)], this.p[dim(mod512, 10)], this.p[dim(mod512, FrameMetricsAggregator.EVERY_DURATION)]);
            h2 = this.p[mod512] ^ h1(this.p[dim(mod512, 12)]);
        } else {
            int[] iArr2 = this.q;
            iArr2[mod512] = iArr2[mod512] + g2(this.q[dim(mod512, 3)], this.q[dim(mod512, 10)], this.q[dim(mod512, FrameMetricsAggregator.EVERY_DURATION)]);
            h2 = this.q[mod512] ^ h2(this.q[dim(mod512, 12)]);
        }
