...
public class Analyzer {
    ...
    private static int getMaxDimensionTraversal(ConstraintWidget widget, int orientation, boolean topLeftFlow, int depth) {
        ...
        Iterator it2 = widget.mListAnchors[startOffset].getResolutionNode().dependents.iterator();
        while (it2.hasNext()) {
            dimensionPre = Math.max(dimensionPre, getMaxDimensionTraversal(((ResolutionAnchor) ((ResolutionNode) it2.next())).myAnchor.mOwner, orientation, topLeftFlow, downDepth));
        }
        Iterator it3 = widget.mListAnchors[endOffset].getResolutionNode().dependents.iterator();
        while (it3.hasNext()) {
            dimensionPost2 = Math.max(dimensionPost2, getMaxDimensionTraversal(((ResolutionAnchor) ((ResolutionNode) it3.next())).myAnchor.mOwner, orientation, topLeftFlow, postTemp + downDepth));
        }
        if (hasBaseline) {
        ...
        if (orientation == 1) {
            Iterator it4 = widget.mBaseline.getResolutionNode().dependents.iterator();
            while (it4.hasNext()) {
                ResolutionAnchor anchor = (ResolutionAnchor) ((ResolutionNode) it4.next());
                if (flow == 1) {
