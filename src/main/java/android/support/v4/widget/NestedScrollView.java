...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent2, NestedScrollingChild2, ScrollingView {
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case 20:
                        if (event.isAltPressed()) {
                            handled = fullScroll(NikonType2MakernoteDirectory.TAG_ADAPTER);
                            break;
                        } else {
                            handled = arrowScroll(NikonType2MakernoteDirectory.TAG_ADAPTER);
                            break;
                        }
                    case 62:
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, NikonType2MakernoteDirectory.TAG_ADAPTER);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(NikonType2MakernoteDirectory.TAG_ADAPTER)) {
                return false;
            }
            return true;
        }
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = NikonType2MakernoteDirectory.TAG_ADAPTER;
        } else if (direction == 1) {
            direction = 33;
        }
